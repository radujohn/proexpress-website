...
export default function HomePage() {
  ...
  // ── CMS content — falls back to hardcoded defaults if DB has nothing ──
  const hero = useCmsContent('home_hero', defaultHero)
  const testimonials = useCmsContent('home_testimonials', defaultTestimonials)
  const faqs = useCmsContent('home_faqs', defaultFAQs)
  const cta = useCmsContent('home_cta', defaultCTA)
          ...
          <div className="max-w-3xl">
            ...
            >
              ...
              <h1 className="font-heading font-900 text-4xl sm:text-5xl lg:text-6xl xl:text-7xl text-white leading-tight mb-6">
                {hero.title1}
                <span className="block text-electric">{hero.title2}</span>
                {hero.title3}
              </h1>
      ...
      <section className="py-16 bg-navy relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 text-center">
          <AnimatedSection>
            ...
            <p className="text-white/60 text-lg mb-8">{cta.subtitle}</p>
